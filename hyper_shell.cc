  ...
  void Solid<dim>::setup_qph_one_cell(
    ...
    PerTaskData_SQPH & /*data*/)
    {
	  ...
	  material_id = 1;
        ...
        lqph[q_point]->setup_lqp(parameters, material_id, outward_normal, dist_from_sc, heaviside_function);
      }
    }
...
int main(int argc, char *argv[])
{
  ...
  Assert(argc == 2, ExcMessage("Expected one argument"));
    const unsigned int dimension = strtol(argv[1],NULL,10);
    std::cout<< "This is a " << dimension << " dimensional problem." << std::endl;
    try
    {
      if(dimension==3)
      {
        Solid<3>         solid("parameters.prm");
        solid.run();
      }
      else if(dimension==2)
      {
